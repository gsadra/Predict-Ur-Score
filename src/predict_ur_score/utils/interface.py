...
class Interface:
    ...
    def predict_sec(self) -> None:
        ...
        self.student_inps["extracurricular_activities"] = (
            self._check_extracurricular_activities()
        )
        self.student_inps["weekly_self_study_hours"] = (
            self._check_weekly_self_study_hours()
        )
        self.student_inps["career_aspiration"] = (
            self._check_career_aspiration()
        )
    ...
    def show_predict(self) -> None:
        ...
        print(
            tabulate(
                rows,
                headers=["Subject", "Predicted Score"],
                tablefmt="psql",
                numalign="center",
                stralign="center",
            )
    ...
    def stat_sec(self) -> None:
        ...
        courses = {
            number + 2: score for number, score in enumerate(stat.TOPICS)
        }
        courses[1] = "all_stat"
    ...
    def _check_part_time_job(self) -> bool:
        ...
        inp = (
            input("\nAre you currently on a part-time job? (yes, no): ")
            .strip()
            .lower()
        )
    ...
    def _check_absence_days(self) -> int:
        try:
            inp = int(
                input(
                    "\nHow many days you may be absent? (ex: 4): "
                ).strip()
            )
        except ValueError:
    ...
    def _check_extracurricular_activities(self) -> bool:
        inp = (
            input("\nDo you have extracurricular activities? (yes, no): ")
            .strip()
            .lower()
        )
        if not inp in self.ANS:
    ...
    def _check_weekly_self_study_hours(self) -> int:
        try:
            inp = int(
                input(
                    "\nHow many hours your going to study by yourself in a week? (ex: 25): "
                ).strip()
            )
        except ValueError:
            raise PredictorInvalidInput("Input is not a number!")
    ...
    def _check_career_aspiration(self) -> str:
        ...
        career_aspirations = {
            number + 1: career
            for number, career in enumerate(eda.career_aspirations())
        }
        ...
        if not inp in career_aspirations:
